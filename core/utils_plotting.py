...
def plot_params(neurons: List[LIF]):
    ...
    axs[1, 2].set_xlim([-75,20])
    axs[1, 2].set_title('threshold')

    axs[2, 0].hist([n.k_j[0] for n in neurons], alpha=0.7)
    axs[2, 0].hist([n.k_j[1] for n in neurons], alpha=0.7)
    axs[2, 0].set_xlim([1e-9, 1])
    axs[2, 0].set_title('k_j')

    axs[2, 1].hist([n.f_j[0] for n in neurons], alpha=0.7)
    axs[2, 1].hist([n.f_j[1] for n in neurons], alpha=0.7)
    axs[2, 1].set_xlim([-200, 200])
    axs[2, 1].set_title('f_j')

    axs[2, 2].hist([n.b_j[0] for n in neurons], alpha=0.7)
    axs[2, 2].hist([n.b_j[1] for n in neurons], alpha=0.7)
    axs[2, 2].set_xlim([-1,1])
    axs[2, 2].set_title('b_j')

def visualize_stc(pca: PCA, dt: float):
    """Plots the principal components computed by pca."""
    ...
    for i in range(n_components):
        plt.plot(time, pca.components_[i, :], label = f"PC{i}: {round(explained_var[i],4)}")
    plt.legend()
    plt.ylabel('spikes')
    plt.xlabel('time (ms)')
...
def visualize_paramvar(neurons: List[LIF], projections):
    ...
    axs[2, 0].scatter([n.k_j[0] for n in neurons], projections[:,0])
    axs[2, 0].scatter([n.k_j[0] for n in neurons], projections[:,1])
    axs[2, 0].scatter([n.k_j[0] for n in neurons], projections[:,2])
    # axs[2, 0].hist([n.k_j[1] for n in neurons], alpha=0.7)
    axs[2, 0].set_xlim([1e-9, 1])
    axs[2, 0].set_title('k_j')

    axs[2, 1].scatter([n.f_j[0] for n in neurons], projections[:,0])
    axs[2, 1].scatter([n.f_j[0] for n in neurons], projections[:,1])
    axs[2, 1].scatter([n.f_j[0] for n in neurons], projections[:,2])
    axs[2, 1].set_xlim([-200, 200])
    axs[2, 1].set_title('f_j')

    axs[2, 2].scatter([n.b_j[0] for n in neurons], projections[:,0])
    axs[2, 2].scatter([n.b_j[0] for n in neurons], projections[:,1])
    axs[2, 2].scatter([n.b_j[0] for n in neurons], projections[:,2])
    axs[2, 2].set_xlim([-1,1])
    axs[2, 2].set_title('b_j')